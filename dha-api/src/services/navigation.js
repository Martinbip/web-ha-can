...
const { getDefaultNavItems } = require('../defaults/default-items');

const NAV_TYPE = 'navigation';
...
async function readRecord() {
  const entries = await getStore().documents(NAV_TYPE).findMany({ limit: 1 });
...
async function update(ctx) {
  ...
  const record = await readRecord();
  const saved = record
    ? await getStore().documents(NAV_TYPE).update({ documentId: record.documentId, data: { items } })
    : await getStore().documents(NAV_TYPE).create({ data: { items } });
