...
const CONTACT_TYPE = 'contactInquiry';
const ORDER_TYPE = 'orderRequest';
...
function getService(config) {
  return getStore().documents(config.sanityType);
}
...
async function buildDashboardCards() {
  ...
  for (const config of listResourceConfigs()) {
    ...
    const count = await getStore().documents(config.sanityType).count({});
...
async function dashboard(ctx) {
  ...
  const [
    ...
    buildDashboardCards(),
    getStore().documents(CONTACT_TYPE).count({ filters: { status: 'new' } }),
    getStore().documents(CONTACT_TYPE).findMany({
      filters: { status: 'new' },
      sort: { createdAt: 'desc' },
      fields: ['name', 'service', 'createdAt'],
      limit: DASHBOARD_PENDING_LIMIT,
    }),
    getStore().documents(ORDER_TYPE).count({ filters: { status: 'new' } }),
    getStore().documents(ORDER_TYPE).findMany({
      filters: { status: 'new' },
      sort: { createdAt: 'desc' },
    ...
    getStore().documents(PRODUCT_TYPE).count({ filters: { in_stock: false } }),
    Promise.all(
      draftConfigs.map((config) =>
        getStore().documents(config.sanityType).count({ filters: { publishedAt: { $null: true } } }),
      ),
    ),
  ]);
